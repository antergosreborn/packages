...
class Location(GtkBaseBox):
    ...
    def select_detected_country(self):
        """ Selects listbox item that matches detected country using online geolocation """

        print("SELECT_DETECTED_COUNTRY START", flush=True)
        ...
        try:
            ...
            logging.debug("Getting your country using ipapi.co")

            print("CALLING IPAPI", flush=True)
            ...
            with urllib.request.urlopen(
                ...
                print("IPAPI DATA:", data, flush=True)

                detected_country = data.get("country_name")

                print("DETECTED COUNTRY:", detected_country, flush=True)

                logging.debug(
                    "Detected country from ipapi: %s",
                    detected_country
                )
            ...
            logging.warning(
                "Could not detect country from ipapi: %s",
                err
            )
        ...
        if detected_country:
            ...
            for listbox_row in self.listbox.get_children():
                ...
                if label is not None:
                    ...
                    if detected_country in label_text:
    ...
    def prepare(self, direction):
        ...
        print("LOCATION: hide_all", flush=True)
        self.hide_all()

        print("LOCATION: fill_listbox", flush=True)
        self.fill_listbox()

        print("LOCATION: select_detected_country", flush=True)
        self.select_detected_country()

        print("LOCATION: translate_ui", flush=True)
        self.translate_ui()

        print("LOCATION: set_sensitive", flush=True)
        self.forward_button.set_sensitive(True)

        print("LOCATION: show_all", flush=True)
        self.show_all()

        print("LOCATION: install_id", flush=True)

        if os.environ.get("CNCHI_DEV_MODE") == "1":
            self.settings.set('install_id', "dev-install-id")
        else:
            self.settings.set('install_id', self.get_and_save_install_id())

        print("LOCATION: done prepare", flush=True)
