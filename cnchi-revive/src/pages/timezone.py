...
class Timezone(GtkBaseBox):
    ...
    def __init__(self, params, prev_page="location", next_page="keymap"):
        ...
        # Setup window
        logging.debug("DEV TEST: about to create TimezoneMap")

        self.tzmap = timezonemap.TimezoneMap()

        logging.debug("DEV TEST: TimezoneMap created")
    ...
    def prepare(self, direction):
        ...
        if (
            os.environ.get("CNCHI_DEV_MODE") != "1"
            and self.autodetected_coords is None
        ):
            try:
    ...
    def start_auto_timezone_process(self):
        """ Starts automatic timezone detection """

        import json

        try:

            logging.debug("Getting timezone from ipapi.co")
            ...
            logging.warning(
                "Could not detect timezone automatically: %s",
                err
            )
