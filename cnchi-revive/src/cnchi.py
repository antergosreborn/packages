...
class CnchiApp(Gtk.Application):
    ...
    TEMP_FOLDER = os.path.expanduser('~/.cache/cnchi-dev')
    ...
    def do_activate(self):
        ...
        if os.getuid() != 0 and os.environ.get("CNCHI_DEV_MODE") != "1":
...
class CnchiInit():
    ...
    LOG_FOLDER = os.path.expanduser('~/.cache/cnchi-dev/logs')
    TEMP_FOLDER = os.path.expanduser('~/.cache/cnchi-dev')
    ...
    def __init__(self):
        ...
        # DEV SAFETY: never edit host /etc/pacman.conf while testing Cnchi
        logging.warning("DEV MODE: skipping host /etc/pacman.conf repository check/write")
    ...
    def enable_repositories():
        ...
        # DEV SAFETY: do not modify the host system pacman.conf while testing
        for repo in repositories:
            if not enabled[repo]:
                logging.warning(
                    "DEV MODE: would add %s repository to /etc/pacman.conf, skipping",
                    repo
                )
