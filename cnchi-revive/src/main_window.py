...
class MainWindow(Gtk.ApplicationWindow):
    ...
    def set_geometry(self):
        ...
        # ===== Cnchi 26: Resizable Main Window =====
        self.set_position(Gtk.WindowPosition.CENTER)
        self.set_resizable(True)

        logging.debug("Setting main window default size to %dx%d",
    ...
    def on_forward_button_clicked(self, _widget, _data=None):
        ...
        if next_page == 'location' and next_page not in self.pages.keys():
            ...
            if os.environ.get("CNCHI_DEV_MODE") == "1":

                print("DEV MODE: lazy loading early pages", flush=True)
                ...
                self.pages["installation_ask"] = pages.ask.InstallationAsk(self.params)

                self.pages["installation_automatic"] = pages.automatic.InstallationAutomatic(self.params)

                self.pages["installation_advanced"] = pages.advanced.InstallationAdvanced(self.params)

                self.pages["installation_zfs"] = pages.zfs.InstallationZFS(self.params)

                self.progressbar_step = 1.0 / 10
        ...
        if next_page is not None:
            ...
            print("BEFORE STORE_VALUES", flush=True)
            stored = self.current_page.store_values()
            print("AFTER STORE_VALUES", flush=True)

            if stored:

                self.set_progressbar_step(self.progressbar_step)

                print("BEFORE REMOVE PAGE", flush=True)
                self.main_box.remove(self.current_page)
                print("AFTER REMOVE PAGE", flush=True)

                self.current_page = self.pages[next_page]
                print("AFTER SET CURRENT PAGE", next_page, flush=True)

                if self.current_page is not None:

                    print("BEFORE PREPARE", next_page, flush=True)
                    self.current_page.prepare('forwards')
                    print("AFTER PREPARE", next_page, flush=True)

                    self.main_box.add(self.current_page)
                    print("AFTER ADD PAGE", next_page, flush=True)
